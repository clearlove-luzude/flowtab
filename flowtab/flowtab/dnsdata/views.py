...
from dnsdata.models import Cdn_bpsdata
from dnsdata.models import Cdn_trafficdata,Cdn_bpsdata,DomainName_tab,Cdn_lookcloud,DomainName_tab_shijie,Controls,Controls_info,Cdn_lookcloud_flow,Domainname_cost,Domainname_yf_cost
...
@login_required
def m_cost(request):
    return render(request,"m_cost_web.html")

@login_required
def day_cost(request):
    return render(request,"day_cost_web.html")


@login_required
def m_yf_cost(request):
    return render(request,"m_yf_cost_web.html")

@login_required
def day_yf_cost(request):
    return render(request,"day_yf_cost_web.html")
...
def m_cost_api(request):
    if request.method == 'GET':
        months = request.GET.get("时间")
    if len(months) < 2:
        cursor = connection.cursor()
        cursor.execute("select DATE_FORMAT(TimeStamp_cst,'%Y%m') as week,sum(Value)/count(0)*9.3,count(0) from dnsdata_domainname_cost group by week;")
        raw = cursor.fetchall()
        #dataCount = raw.count()  # 数据总数
        lis = []
        for i in raw:
            dict = {}
            dict['时间'] = i[0]
            dict['value'] = float('%.2f' %(i[1]/1000000))  #
            dict['天数'] = i[2]
            lis.append(dict)
    else:
        cursor = connection.cursor()
        cursor.execute("select DATE_FORMAT(TimeStamp_cst,'%%Y%%m') as week,sum(Value)/count(0)*9.3,count(0) from dnsdata_domainname_cost where DATE_FORMAT(TimeStamp_cst,'%%Y%%m') = '%s'  group by week;"%months)
        raw = cursor.fetchall()
        #dataCount = raw.count()  # 数据总数
        lis = []
        for i in raw:
    ...
    for i in contacts:
        res.append(i)
    print(res)
    Result = {"code": 0, "msg": "", "count": len(lis), "data": res}
    # json.dumps(Result, cls=DateEncoder)没有时间字段问题可直接返回此代码。有就返回下面代码
    return HttpResponse(json.dumps(Result, cls=DateEncoder), content_type="application/json")


def m_yf_cost_api(request):
    if request.method == 'GET':
        months = request.GET.get("时间")
    if len(months) < 2:
        cursor = connection.cursor()
        cursor.execute("select DATE_FORMAT(TimeStamp_cst,'%Y%m') as week,sum(Value)/count(0)*7.5,count(0) from dnsdata_domainname_yf_cost group by week;")
        raw = cursor.fetchall()
        #dataCount = raw.count()  # 数据总数
        lis = []
        for i in raw:
            dict = {}
            dict['时间'] = i[0]
            dict['value'] = float('%.2f' %(i[1]))  #
            dict['天数'] = i[2]
            lis.append(dict)
    else:
        cursor = connection.cursor()
        cursor.execute("select DATE_FORMAT(TimeStamp_cst,'%%Y%%m') as week,sum(Value)/count(0)*7.5,count(0) from dnsdata_domainname_yf_cost where DATE_FORMAT(TimeStamp_cst,'%%Y%%m') = '%s'  group by week;"%months)
        raw = cursor.fetchall()
        #dataCount = raw.count()  # 数据总数
        lis = []
        for i in raw:
...
def day_cost_api(request):
    ...
    if len(months) < 2:
        ...
        cursor = connection.cursor()
        cursor.execute("select Value,DATE_FORMAT(TimeStamp_cst,'%%Y-%%m-%%d %%H:%%i:%%S') as TimeStamp_cst from dnsdata_domainname_cost where DATE_FORMAT(TimeStamp_cst,'%%Y%%m') = '%s' order by TimeStamp_cst;" % months)
        raw = cursor.fetchall()
...
@csrf_exempt
def day_yf_cost_api(request):
    if request.method == 'GET':
        months = request.GET.get("时间")
    print(months)
    if len(months) < 2:
        dates = Domainname_yf_cost.objects.all().values_list('id', 'Value', 'TimeStamp_cst')
        dataCount = dates.count()  # 数据总数
        lis = []
        for i in dates:
             dict = {}
        ...
        cursor = connection.cursor()
        cursor.execute("select Value,DATE_FORMAT(TimeStamp_cst,'%%Y-%%m-%%d %%H:%%i:%%S') as TimeStamp_cst from dnsdata_domainname_yf_cost where DATE_FORMAT(TimeStamp_cst,'%%Y%%m') = '%s';" % months)
        raw = cursor.fetchall()
        dataCount = len(raw)  # 数据总数
        lis = []
        for i in raw:
